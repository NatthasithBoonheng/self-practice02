...
function loadQuotes() {
  const savedQuotes = JSON.parse(localStorage.getItem("Load") || "[]");

  savedQuotes.forEach((x) => {
    const newQuote = addQuote(x.content, x.author);
    addQuoteToDOM(newQuote);});
}
function saveQuotes() {
  localStorage.setItem("Load",JSON.stringify(getAllQuotes()))
}
